...
export default function NewsCard({ post, isFeatured = false }: NewsCardProps) {
      ...
      <div className="mb-8 max-w-4xl mx-auto">
        <div className="group relative overflow-hidden bg-metal-darker border-2 border-metal-gray hover:border-metal-red transition-all duration-300">
          <div className="grid md:grid-cols-2 gap-0">
            {/* Featured Image */}
            <div className="relative w-full overflow-hidden bg-metal-darker max-h-[300px] md:max-h-[350px]">
              ...
              {post.videoUrl && (
                <div className="absolute inset-0 flex items-center justify-center bg-black/20 group-hover:bg-black/30 transition-colors pointer-events-none">
                  <a
                    ...
                    className="bg-white/90 hover:bg-white text-black rounded-full p-2 transition-all duration-200 hover:scale-110 pointer-events-auto"
               ...
               <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-2">
                 <Link
                   href={`/news/${post.slug}`}
                   className="bg-metal-red text-white px-2 py-1 font-bold uppercase tracking-wider text-xs hover:bg-red-600 hover:scale-105 hover:shadow-md hover:shadow-metal-red/30 transition-all duration-200 whitespace-nowrap text-center"
                 >
                   Read More
                 </Link>
                {post.externalLink && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="bg-black/30 border border-metal-red text-metal-light hover:bg-metal-red hover:text-white px-2 py-1 font-bold uppercase tracking-wider text-xs transition-all whitespace-nowrap text-center flex items-center justify-center gap-1"
                  >
            ...
            <div className="p-4 md:p-6 flex flex-col justify-center relative z-10">
              <div className="mb-3">
                ...
                <Link href={`/news/${post.slug}`} className="block">
                  <h2 className="text-xl font-bold text-metal-light group-hover:text-metal-red transition-colors mb-3 uppercase tracking-tight cursor-pointer">
                    {post.title}
                  </h2>
                </Link>
              ...
              <div className="mt-4">
                <Link
                  href={`/news/${post.slug}`}
                  className="inline-flex items-center gap-2 text-metal-red font-bold uppercase tracking-wider text-sm group-hover:gap-3 transition-all duration-300"
                >
                  Read More
          ...
          {/* Hover accent line */}
          <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-metal-red transform scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left" />
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="group relative h-full flex flex-col overflow-hidden bg-metal-darker border-2 border-metal-gray hover:border-metal-red transition-all duration-300">
      {/* Post Image */}
      <div className="relative w-full overflow-hidden bg-metal-darker flex-shrink-0">
      ...
      <div className="p-6 relative z-10 flex-grow flex flex-col">
        ...
        <Link href={`/news/${post.slug}`} className="block">
          <h3 className="text-xl font-bold text-metal-light group-hover:text-metal-red transition-colors mb-3 uppercase tracking-tight cursor-pointer">
            {post.title}
          </h3>
        </Link>
        <p className="text-sm text-metal-light opacity-90 line-clamp-2 flex-grow">
          {post.excerpt}
        </p>
      </div>
